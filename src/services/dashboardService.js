...
export async function getDashboardStats() {
  ...
  const stats = {
    ...
    active: data.filter((i) => i.status === "In Progress").length,

    completed: data.filter((i) => i.status === "Completed").length,

    notStarted: data.filter((i) => i.status === "Not Started").length,

    overdue: data.filter((i) => {
      if (!i.due_date) return false;

      return new Date(i.due_date) < today && i.status !== "Completed";
    }).length,
  };
  ...
  data.forEach((initiative) => {
    const name = initiative.departments?.name ?? "Unassigned";
    ...
    departmentHealth[name].totalProgress += initiative.progress ?? 0;

    departmentHealth[name].initiatives++;
  });

  const departments = Object.entries(departmentHealth).map(([name, value]) => ({
    name,
    health: Math.round(value.totalProgress / value.initiatives),
  }));
